...
def create_input():
    ...
    feature_layer_inputs['age'] = tf.keras.Input(shape=(None,), name='age')

    for col in NUMERIC_COLUMNS:
        temp_obj = Preprocess(col)
        num_col = temp_obj.num_column()
        feature_columns.append(num_col)
        feature_layer_inputs[col] = tf.keras.Input(shape=(None,), name=col)
    ...
    for col in CATEGORICAL_COLUMNS:
        ...
        feature_layer_inputs[col] = tf.keras.Input(shape=(None,), name=col, dtype=tf.dtypes.int64)
    return feature_columns, feature_layer_inputs


model = create_model()
tf.keras.utils.plot_model(model, 'my_first_model1.png', show_shapes=True)
